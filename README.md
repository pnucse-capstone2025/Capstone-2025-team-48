...
1. **웹 기반 접근성** – 단순히 웹 브라우저와 인터넷만 있으면 별도의 장비나 고비용 라이선스 없이도 누구나 쉽게 분석을 수행할 수 있습니다.  
2. **경량화된 AI 모델** – YOLOv8n, MobileNet, LSTM을 적절히 결합하여 높은 정확도를 유지하면서도 처리 속도와 비용을 최적화했습니다.  
3. **분석 속도 개선** – ByteTrack을 통한 추적 기법을 활용하여 기존 방식 대비 약 5.7배 빠른 속도를 구현했으며, 실시간에 가까운 피드백을 제공합니다.  
4. **맞춤형 분석 기능** – 선수 위치, 팀 간 간격, 포메이션 밀집도 등 실제 전술 분석에 필요한 지표를 제공하며, 단순 이벤트 기록을 넘어선 **전술적 맥락 분석**이 가능합니다.  

따라서 본 프로젝트는 기존 서비스 대비 **저비용·고효율·사용자 친화적**이라는 측면에서 뚜렷한 차별성을 가집니다.  
