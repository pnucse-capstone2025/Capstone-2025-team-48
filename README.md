...
- **사용자 인터페이스(User Interface)** : 사용자 인터페이스(UI)는 사람과 컴퓨터 시스템 사이의 상호작용을 하는 시스템이다. 사용자가 기기를 효율적으로 제어하고 시스템과 소통할 수 있도록 돕는다. 여기에는 키보드, 마우스와 같은 물리적 요소뿐만 아니라 화면의 아이콘, 버튼, 메뉴, 텍스트, 그리고 동작 인식과 같은 가상적 요소들이 모두 포함된다. 사용자 인터페이스는 사용 편의성을 높이고 오류를 줄이며 정보 접근을 용이하게 하는 데 중점을 두고 있다.
...
# 4. 개발 결과
## 4.1. 전체 시스템 흐름도
> 사용자가 축구 영상을 웹 페이지에 업로드한다. 업로드 된 영상을 분석 설정 부분에서 Home팀과 Away팀, 심판을 설정하고 분석 시작 버튼을 눌러 축구 전술 분석을 시작한다. 축구 영상 분석이 완료되면, 분석 결과에서 분석이 완료된 축구 영상이 나옵니다. 그 영상에는 실시간으로 축구 선수들의 위치 데이터와 포메이션에 따른 전술 분석이 나옵니다. 마지막으로 분석이 완료된 축구 영상을 다운로드하여 개인 소장이 가능합니다.
...
> 프로젝트 진행 과정에서 산업체 멘토링을 통해 여러 차례 피드백을 받았으며, 이를 실제 구현 단계에서 적극적으로 반영하였다. 우선, 정확도 향상에 대한 조언을 바탕으로 기존 YOLO 모델을 그대로 사용하는 대신, 축구 경기 데이터셋에 맞추어 Fine-Tuning을 진행하였다. 이를 통해 선수 탐지 정확도가 기존 대비 약 28% 개선되었고, 실제 경기 상황에서도 안정적으로 선수 식별이 가능해졌다.
...
## 7.2. 팀원 별 참여 후기
> 본 연구에서는 축구 경기 영상을 기반으로 선수의 탐지와 추적, 팀 식별, 그리고 전술 예측까지를 수행할 수 있는 통합 분석 시스템을 구현하였다. YOLO를 활용한 객체 탐지를 통해 선수의 위치 정보를 확보하고, MobileNet을 통해 팀 식별을 시도하였으며, LSTM을 통해 시계열 데이터를 활용한 전술적 패턴 예측을 가능하게 하였다. 이를 통해 기존 수작업 위주의 전술 분석 방식보다 효율적이고 체계적인 분석이 가능하다는 점을 확인할 수 있었다. 또한, 웹 기반 서비스로 구현하여 사용자가 직관적으로 영상을 업로드하고 결과를 확인할 수 있도록 하여, 소규모 팀이나 아마추어 구단에서도 손쉽게 활용할 수 있는 시스템적 기반을 마련하였다.
그러나 본 연구는 몇 가지 한계점을 지닌다. 첫째, 팀 식별 정확도가 상대적으로 낮아 경기 상황에 따라 전술 분석 결과의 신뢰도가 떨어질 수 있다. 둘째, 모델 최적화에도 불구하고 분석 소요 시간이 길어 실시간 분석에는 제약이 존재한다. 셋째, 현재는 제한된 데이터셋을 기반으로 학습되었기 때문에 다양한 경기 환경과 카메라 촬영 조건에서의 일반화 성능을 보장하기 어렵다.
>
>향후 연구에서는 다음과 같은 개선 방향을 고려할 수 있다.
1.	데이터 확장 및 고도화: 다양한 경기 환경(조명, 카메라 앵글, 유니폼 색상 등)에서 수집된 대규모 데이터를 확보하여 모델의 일반화 성능을 높인다.
2.	다중 모달 데이터 활용: 영상 데이터뿐만 아니라 센서 기반 위치 데이터(GPS, IMU 등)와 결합하여 보다 정확한 전술 분석을 가능하게 한다.
3.	실시간 분석 최적화: 경량화 모델과 GPU 최적화, 분산 처리 기법 등을 도입하여 실시간 전술 분석이 가능하도록 성능을 개선한다.
4.	전술 해석 자동화: 단순히 포지션과 이동 궤적을 제시하는 수준을 넘어, 패스 네트워크 분석, 압박 강도 측정, 팀 간 간격 유지 등 고차원적인 전술 지표를 자동 산출할 수 있는 알고리즘을 개발한다.
종합적으로, 본 연구는 축구 경기 영상 정보를 활용한 전술 분석의 가능성을 제시하고 실제 구현을 통해 그 실효성을 검증하였다는 점에서 의미가 있다. 앞으로의 연구에서는 정확도 향상, 실시간성 확보, 고차원적 분석 기능 추가를 통해 더 넓은 범위에서 활용될 수 있는 실질적인 스포츠 분석 도구로 발전할 수 있을 것이다.
